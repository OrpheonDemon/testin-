...
<body>
        ...
        <div class="contenido">
            <div class="tabla-crud">
                <div class="tabla-header">
                    ...
                    <button id="btnInsertar">Insertar Usuario</button>
                </div>
                ...
                <div id="modalUsuario" class="modal" style="display:none;">
                    <div class="modal-content">
                        ...
                        <h2 id="modalTitulo">Insertar Usuario</h2>

                        <form method="POST" action="../Backend/CRUDs/usuariosCRUD.php?accion=guardar">
                            <input type="hidden" name="IdUsuario" id="formIdUsuario">

                            <label>Nombres:</label>
                            <input type="text" name="Nombres" id="formNombres" required>

                            <label>Apellido Paterno:</label>
                            <input type="text" name="ApellidoPat" id="formApellidoPat" required>

                            <label>Apellido Materno:</label>
                            <input type="text" name="ApellidoMat" id="formApellidoMat">

                            <label>Correo:</label>
                            <input type="email" name="Correo" id="formCorreo" required>

                            <label>Celular:</label>
                            <input type="text" name="Celular" id="formCelular">

                            <label>Password:</label>
                            <input type="password" name="Password" id="formPassword">

                            <label>Rol:</label>
                            <select name="IdRol" id="formRol" required>
                                <option value="">Seleccione un rol</option>
                                <?php
                                require_once '../Backend/conexion.php';
                                $resRoles = $link->query("SELECT IdRol, Nombre FROM TRoles WHERE Estado = 1");
                            ...
                            <label>Grado:</label>
                            <select name="IdGrado" id="formGrado" required>
                                <option value="">Seleccione un grado</option>
                                <?php
                                $resGrados = $link->query("SELECT IdGrado, Nombre FROM TGrados WHERE Estado = 1");
                                while ($grado = $resGrados->fetch_assoc()) {
                ...
                <table style="width:100%; margin-top:10px;">
                    ...
                    <tbody>
                        <?php
                        $res = $link->query("
                    SELECT u.IdUsuario, u.Nombres, u.ApellidoPat, u.ApellidoMat, u.Correo, u.Celular,
                           r.Nombre as Rol, g.Nombre as Grado, u.Estado
                    FROM TUsuarios u
                ...
                ");
                        ...
                        while ($row = $res->fetch_assoc()) {
                            ...
                            $estadoText = $row['Estado'] == 1 ? 'Habilitado' : ($row['Estado'] == 2 ? 'Cancelado' : 'Pendiente');
                            $contador++;
                            echo "<tr>
                        <td>$contador</td>
                        ...
                        <td>$estadoText</td>
                        <td>
                            <button class='btnEditar' data-id='{$row['IdUsuario']}'>Editar</button>
                            <button class='btnBorrar' data-id='{$row['IdUsuario']}'>Borrar</button>
                        </td>
                    </tr>";
                        }
                        ?>
    ...
    <script>
        ...
        btnInsertar.addEventListener('click', () => {
            ...
            modalTitulo.textContent = 'Insertar Usuario';
        ...
        document.querySelectorAll('.btnEditar').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                fetch(`../Backend/CRUDs/usuariosCRUD.php?accion=get&id=${id}`)
                    .then(res => res.json())
                    .then(data => {
                        modal.style.display = 'flex';
                        modalTitulo.textContent = 'Editar Usuario';
                        document.getElementById('formIdUsuario').value = data.IdUsuario;
                        document.getElementById('formNombres').value = data.Nombres;
                        document.getElementById('formApellidoPat').value = data.ApellidoPat;
                        document.getElementById('formApellidoMat').value = data.ApellidoMat;
                        document.getElementById('formCorreo').value = data.Correo;
                        document.getElementById('formCelular').value = data.Celular;
                        document.getElementById('formRol').value = data.IdRol;
                        document.getElementById('formGrado').value = data.IdGrado;
                    });
            });
        });
        ...
        document.querySelectorAll('.btnBorrar').forEach(btn => {
            btn.addEventListener('click', () => {
                if (confirm("¿Seguro que quieres borrar este usuario?")) {
                    const id = btn.getAttribute('data-id');
                    window.location.href = `../Backend/CRUDs/usuariosCRUD.php?accion=borrar&id=${id}`;
                }
            });
        });
    </script>
