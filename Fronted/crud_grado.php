...
<body>
        ...
        <div class="contenido">
            <div class="tabla-crud">
                <div class="tabla-header">
                    ...
                    <button id="btnInsertar">Insertar Grado</button>
                </div>

                <!-- Modal -->
                <div id="modalGrado" class="modal" style="display:none;">
                    <div class="modal-content">
                        <span class="close">&times;</span>
                        <h2 id="modalTitulo">Insertar Grado</h2>

                        <form method="POST" action="../Backend/CRUDs/gradosCRUD.php?accion=guardar">
                            <input type="hidden" name="IdGrado" id="formIdGrado">
                ...
                <table style="width:100%; margin-top:10px;">
                    ...
                    <tbody>
                        ...
                        $res = $link->query("
                            SELECT IdGrado, Nombre, Descripcion, Estado, FechaCreacion
                            FROM TGrados
                            WHERE Estado = 1
                        ");
                        $contador=0;
                        ...
                        <td>
                            <button class='btnEditar' data-id='{$row['IdGrado']}'>Editar</button>
                            <button class='btnBorrar' data-id='{$row['IdGrado']}'>Borrar</button>
                        </td>
                    </tr>";
                        }
                        ?>
    ...
    <script>
        ...
        const modal = document.getElementById('modalGrado');
        ...
        btnInsertar.addEventListener('click', () => {
            ...
            modalTitulo.textContent = 'Insertar Grado';
            modal.querySelector('form').reset();
            document.getElementById('formIdGrado').value = '';
        });
        ...
        document.querySelectorAll('.btnEditar').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                fetch(`../Backend/CRUDs/gradosCRUD.php?accion=get&id=${id}`)
                    .then(res => res.json())
                    .then(data => {
                        modal.style.display = 'flex';
                        modalTitulo.textContent = 'Editar Grado';
                        document.getElementById('formIdGrado').value = data.IdGrado;
        ...
        document.querySelectorAll('.btnBorrar').forEach(btn => {
            btn.addEventListener('click', () => {
                if (confirm("¿Seguro que quieres borrar este grado?")) {
                    const id = btn.getAttribute('data-id');
                    window.location.href = `../Backend/CRUDs/gradosCRUD.php?accion=borrar&id=${id}`;
                }
            });
        });
    </script>
