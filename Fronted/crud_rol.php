...
<body>
        ...
        <div class="contenido">
            <div class="tabla-crud">
                <div class="tabla-header">
                    ...
                    <button id="btnInsertar">Insertar Rol</button>
                </div>
                ...
                <div id="modalRol" class="modal" style="display:none;">
                    <div class="modal-content">
                        ...
                        <h2 id="modalTitulo">Insertar Rol</h2>

                        <form method="POST" action="../Backend/CRUDs/rolesCRUD.php?accion=guardar">
                            <input type="hidden" name="IdRol" id="formIdRol">
                ...
                <table style="width:100%; margin-top:10px;">
                    ...
                    <tbody>
                        ...
                        $res = $link->query("
                            SELECT IdRol, Nombre, Descripcion, Estado, FechaCreacion
                            FROM TRoles
                            WHERE Estado = 1
                        ");
                        $contador=0;
                        ...
                        <td>
                            <button class='btnEditar' data-id='{$row['IdRol']}'>Editar</button>
                            <button class='btnBorrar' data-id='{$row['IdRol']}'>Borrar</button>
                        </td>
                    </tr>";
                        }
                        ?>
    ...
    <script>
        ...
        btnInsertar.addEventListener('click', () => {
            ...
            modalTitulo.textContent = 'Insertar Rol';
            modal.querySelector('form').reset();
            document.getElementById('formIdRol').value = '';
        });
        ...
        document.querySelectorAll('.btnEditar').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                fetch(`../Backend/CRUDs/rolesCRUD.php?accion=get&id=${id}`)
                    .then(res => res.json())
                    .then(data => {
                        modal.style.display = 'flex';
                        modalTitulo.textContent = 'Editar Rol';
                        document.getElementById('formIdRol').value = data.IdRol;
        ...
        document.querySelectorAll('.btnBorrar').forEach(btn => {
            btn.addEventListener('click', () => {
                if (confirm("¿Seguro que quieres borrar este rol?")) {
                    const id = btn.getAttribute('data-id');
                    window.location.href = `../Backend/CRUDs/rolesCRUD.php?accion=borrar&id=${id}`;
                }
            });
        });
    </script>
