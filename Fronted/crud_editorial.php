...
<body>
        ...
        <div class="contenido">
            <div class="tabla-crud">
                <div class="tabla-header">
                    ...
                    <button id="btnInsertar">Insertar Editorial</button>
                </div>

                <!-- Modal -->
                <div id="modalEditorial" class="modal" style="display:none;">
                    <div class="modal-content">
                        <span class="close">&times;</span>
                        <h2 id="modalTitulo">Insertar Editorial</h2>

                        <form method="POST" action="../Backend/CRUDs/editorialesCRUD.php?accion=guardar">
                            <input type="hidden" name="IdEditorial" id="formIdEditorial">
                ...
                <table style="width:100%; margin-top:20px;">
                    ...
                    <tbody>
                        ...
                        $res = $link->query("
                            SELECT IdEditorial, Nombre, Estado, FechaCreacion
                            FROM TEditoriales
                            WHERE Estado = 1
                        ");
                        $contador=0;
                        ...
                        <td>
                            <button class='btnEditar' data-id='{$row['IdEditorial']}'>Editar</button>
                            <button class='btnBorrar' data-id='{$row['IdEditorial']}'>Borrar</button>
                        </td>
                    </tr>";
                        }
                        ?>
    ...
    <script>
        ...
        const modal = document.getElementById('modalEditorial');
        ...
        btnInsertar.addEventListener('click', () => {
            ...
            modalTitulo.textContent = 'Insertar Editorial';
            modal.querySelector('form').reset();
            document.getElementById('formIdEditorial').value = '';
        });
        ...
        document.querySelectorAll('.btnEditar').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                fetch(`../Backend/CRUDs/editorialesCRUD.php?accion=get&id=${id}`)
                    .then(res => res.json())
                    .then(data => {
                        modal.style.display = 'flex';
                        modalTitulo.textContent = 'Editar Editorial';
                        document.getElementById('formIdEditorial').value = data.IdEditorial;
        ...
        document.querySelectorAll('.btnBorrar').forEach(btn => {
            btn.addEventListener('click', () => {
                if (confirm("¿Seguro que quieres borrar esta editorial?")) {
                    const id = btn.getAttribute('data-id');
                    window.location.href = `../Backend/CRUDs/editorialesCRUD.php?accion=borrar&id=${id}`;
                }
            });
        });
    </script>
