...
<body>
        ...
        <div class="contenido">
            <div class="tabla-crud">
                <div class="tabla-header">
                    ...
                    <button id="btnInsertar">Insertar Autor</button>
                </div>
                ...
                <div id="modalAutor" class="modal" style="display:none;">
                    <div class="modal-content">
                        ...
                        <h2 id="modalTitulo">Insertar Autor</h2>

                        <form method="POST" action="../Backend/CRUDs/autoresCRUD.php?accion=guardar">
                            <input type="hidden" name="IdAutor" id="formIdAutor">
                ...
                <table style="width:100%; margin-top:10px;">
                    ...
                    <tbody>
                        ...
                        $res = $link->query("
                    SELECT IdAutor, Nombre, Estado, FechaCreacion
                    FROM TAutores
                    WHERE Estado = 1
                ");
                        $contador=0;
                        while ($row = $res->fetch_assoc()) {
                            $estadoText = $row['Estado'] == 1 ? 'Habilitado' : ($row['Estado'] == 0 ? 'Inahibilitado' : 'Pendiente');
                            $contador++;
                            echo "<tr>
                        <td>$contador</td>
                        <td>{$row['Nombre']}</td>
                        <td>$estadoText</td>
                        <td>
                            <button class='btnEditar' data-id='{$row['IdAutor']}'>Editar</button>
                            <button class='btnBorrar' data-id='{$row['IdAutor']}'>Borrar</button>
                        </td>
                        </tr>";
                        }
                        ?>
    ...
    <script>
        ...
        btnInsertar.addEventListener('click', () => {
            ...
            modalTitulo.textContent = 'Insertar Autor';
            modal.querySelector('form').reset();
            document.getElementById('formIdAutor').value = '';
        });
        ...
        document.querySelectorAll('.btnEditar').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                fetch(`../Backend/CRUDs/autoresCRUD.php?accion=get&id=${id}`)
                    .then(res => res.json())
                    .then(data => {
                        modal.style.display = 'flex';
                        modalTitulo.textContent = 'Editar Autor';
                        document.getElementById('formIdAutor').value = data.IdAutor;
        ...
        document.querySelectorAll('.btnBorrar').forEach(btn => {
            btn.addEventListener('click', () => {
                if (confirm("¿Seguro que quieres borrar este Autor?")) {
                    const id = btn.getAttribute('data-id');
                    window.location.href = `../Backend/CRUDs/autoresCRUD.php?accion=borrar&id=${id}`;
                }
            });
        });
    </script>
