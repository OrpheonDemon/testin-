...
<body>
        ...
        <div class="contenido">
            <div class="tabla-crud">
                <div class="tabla-header">
                    ...
                    <button id="btnInsertar">Insertar Ubicacion</button>
                </div>
                ...
                <div id="modalUbicacion" class="modal" style="display:none;">
                    <div class="modal-content">
                        ...
                        <h2 id="modalTitulo">Insertar Ubicacion</h2>

                        <form method="POST" action="../Backend/CRUDs/ubicacionesCRUD.php?accion=guardar">
                            <input type="hidden" name="IdUbicacion" id="formIdUbicacion">

                            <label>Area:</label>
                            <input type="text" name="Area" id="formArea" required><br>

                            <label>Piso (OPCIONAL):</label>
                            <input type="number" name="Piso" id="formPiso" required><br>

                            <label>Bloque (OPCIONAL):</label>
                            <input type="text" name="Bloque" id="formBloque"><br>

                            <label>Referencia:</label><br>
                            <textarea name="Referencia" id="formReferencia" cols="40" rows="5"></textarea>
                ...
                <table style="width:100%; margin-top:10px;">
                    ...
                    <tbody>
                        ...
                        $res = $link->query("
                        SELECT IdUbicacion, Area, Piso, Bloque, Referencia, Estado, FechaCreacion
                            FROM TUbicaciones
                            WHERE Estado = 1
                        ");
                        $contador=0;
                        ...
                        <td>
                            <button class='btnEditar' data-id='{$row['IdUbicacion']}'>Editar</button>
                            <button class='btnBorrar' data-id='{$row['IdUbicacion']}'>Borrar</button>
                        </td>
                    </tr>";
                        }
                        ?>
    ...
    <script>
        ...
        btnInsertar.addEventListener('click', () => {
            ...
            modalTitulo.textContent = 'Insertar Ubicacion';
            modal.querySelector('form').reset();
            document.getElementById('formIdUbicacion').value = '';
        });
        ...
        document.querySelectorAll('.btnEditar').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                fetch(`../Backend/CRUDs/ubicacionesCRUD.php?accion=get&id=${id}`)
                    .then(res => res.json())
                    .then(data => {
                        modal.style.display = 'flex';
                        modalTitulo.textContent = 'Editar Ubicacion';
                        document.getElementById('formIdUbicacion').value = data.IdUbicacion;
                        document.getElementById('formArea').value = data.Area;
                        document.getElementById('formPiso').value = data.Piso;
                        document.getElementById('formBloque').value = data.Bloque;
                        document.getElementById('formReferencia').value = data.Referencia;
                    });
            });
        });
        ...
        document.querySelectorAll('.btnBorrar').forEach(btn => {
            btn.addEventListener('click', () => {
                if (confirm("¿Seguro que quieres borrar esta ubicacion?")) {
                    const id = btn.getAttribute('data-id');
                    window.location.href = `../Backend/CRUDs/ubicacionesCRUD.php?accion=borrar&id=${id}`;
                }
            });
        });
    </script>
