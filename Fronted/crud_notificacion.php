...
<body>
        ...
        <div class="contenido">
            <div class="tabla-crud">
                <div class="tabla-header">
                    ...
                    <button id="btnInsertar">Insertar Notificación</button>
                </div>
                ...
                <div id="modalNotificacion" class="modal" style="display:none;">
                    <div class="modal-content">
                        ...
                        <h2 id="modalTitulo">Insertar Notificacion</h2>

                        <form method="POST" action="../Backend/CRUDs/notificacionesCRUD.php?accion=guardar">
                            <input type="hidden" name="IdNotificacion" id="formIdNotificacion">
                            ...
                            <label>Mensaje:</label>
                            <textarea name="Mensaje" id="formMensaje" cols="40" rows="5"></textarea>
                ...
                <table style="width:100%; margin-top:10px;">
                    ...
                    LEFT JOIN TUsuarios u ON n.IdUsuario = u.IdUsuario
                    WHERE n.Estado != 2
                ");
                        $contador=0;
                        while ($row = $res->fetch_assoc()) {
                            $estadoText = $row['Estado'] == 1 ? 'Leida' : ($row['Estado'] == 0 ? 'No Leido' : 'Borrado');
                            $contador++;
                            echo "<tr>
                        <td>$contador</td>
                        <td>{$row['Usuario']}</td>
                        ...
                        <td>
                            <button class='btnEditar' data-id='{$row['IdNotificacion']}'>Editar</button>
                            <button class='btnBorrar' data-id='{$row['IdNotificacion']}'>Borrar</button>
                        </td>
                    </tr>";
                        }
                        ?>
    ...
    <script>
        ...
        btnInsertar.addEventListener('click', () => {
            ...
            modalTitulo.textContent = 'Insertar Notificacion';
            modal.querySelector('form').reset();
            document.getElementById('formIdNotificacion').value = '';
        });
        ...
        document.querySelectorAll('.btnEditar').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                fetch(`../Backend/CRUDs/notificacionesCRUD.php?accion=get&id=${id}`)
                    .then(res => res.json())
                    .then(data => {
                        modal.style.display = 'flex';
                        modalTitulo.textContent = 'Editar Notificacion';
                        document.getElementById('formIdNotificacion').value = data.IdNotificacion;
                        document.getElementById('formUsuario').value = data.IdUsuario;
                        document.getElementById('formMensaje').value = data.Mensaje;
                    });
            });
        });
        ...
        document.querySelectorAll('.btnBorrar').forEach(btn => {
            btn.addEventListener('click', () => {
                if (confirm("¿Seguro que quieres borrar este notificacion?")) {
                    const id = btn.getAttribute('data-id');
                    window.location.href = `../Backend/CRUDs/notificacionesCRUD.php?accion=borrar&id=${id}`;
                }
            });
        });
    </script>
